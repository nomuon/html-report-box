...
- **Ports & Adapters**: `core/src/ports.ts` の IF（ReportRepository / SearchIndex / ObjectStorage / AuthVerifier / SessionAuth …）に local/aws の 2 実装。同一 Hono アプリがローカル Bun と Lambda(Node 22) 両方で動く
...
- **Google 認証（2026-07-11 追加）**: `GOOGLE_CLIENT_ID` 設定時、dev サーバーが実 Google 認証（GIS）に切替。詳細は `mem:google_auth`
